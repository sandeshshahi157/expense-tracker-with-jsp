...
<%
    ...
    String email_of_user = null;
    
    double monthlyTotal =0.0;
    String month = null;
    // Map to store monthly totals
    Map<String, Double> monthlyTotalsMap = new HashMap<>();
    ...
    try {
        ...
        // Get monthly totals for the logged-in user
        String monthlyTotalQuery = "SELECT DATE_FORMAT(date, '%Y-%m') AS month, SUM(amount) AS monthlyTotal " +
                "FROM " + logInUsername +
                " WHERE username = ? GROUP BY month";
        dbStatements = dbConnection.prepareStatement(monthlyTotalQuery);
        dbStatements.setString(1, logInUsername);
        dbResultSets = dbStatements.executeQuery();

        while (dbResultSets.next()) {
           month = dbResultSets.getString("month");
           monthlyTotal = dbResultSets.getDouble("monthlyTotal");

            monthlyTotalsMap.put(month, monthlyTotal);
        }

        
                     String usernamequry = "SELECT  first_name as fname,last_name as lname, email as email_id  " +
                "FROM users" +
                " WHERE username = ? GROUP BY username";
        dbStatements = dbConnection.prepareStatement(usernamequry);
        dbStatements.setString(1, logInUsername);
        dbResultSets = dbStatements.executeQuery();

        while (dbResultSets.next()) {
           email_of_user = dbResultSets.getString("email_id");
            last_name = dbResultSets.getString("lname");
             first_name = dbResultSets.getString("fname");
    ...
    String jsonMonthlyData = new Gson().toJson(monthlyTotalsMap);
%>
                ...
                <div class="flex-1 h-screen " >
                    ...
                    <main class="h-screen">
                        ...
                        <div class="flex items-center justify-between px-4 py-4 border-b lg:py-6 dark:border-primary-darker">
                            <h1 class="text-2xl font-mono">Profile</h1>
                        ...
                        <div class="container mx-auto m-20 antialiased dark:text-gray-300">
                            <div>

                                <div class=" dark:text-gray-300 relative  w-5/6 md:w-5/6  lg:w-4/6 xl:w-3/6 mx-auto">
                                    ...
                                    <div class="mt-16">
                                        <h1 class="font-bold dark:text-gray-300 text-center text-3xl text-gray-900"><%= first_name %> <%= last_name %></h1>
                                        <p class="text-center text-sm text-gray-400 font-medium"> <%= email_of_user %></p>
                                        <p>
                                            <span>
                                        ...
                                        <div class="my-5 px-6">
                                            <a href="#" class="text-gray-200 block rounded-lg text-center font-medium leading-6 px-6 py-3 bg-gray-900 hover:bg-black hover:text-white">Connect with <span class="font-bold">@hackeath</span></a>
                                        </div>
                                        <div class="flex justify-between items-center my-5 px-6">
                                            <a href="" class="text-gray-500 dark:text-gray-300 hover:text-gray-900 hover:bg-gray-100 rounded transition duration-150 ease-in font-medium text-sm text-center w-full py-3">Facebook</a>
                                            <a href="" class="text-gray-500 dark:text-gray-300 hover:text-gray-900 hover:bg-gray-100 rounded transition duration-150 ease-in font-medium text-sm text-center w-full py-3">Twitter</a>
                                            <a href="" class="text-gray-500 dark:text-gray-300 hover:text-gray-900 hover:bg-gray-100 rounded transition duration-150 ease-in font-medium text-sm text-center w-full py-3">Instagram</a>
                                            <a href="" class="text-gray-500 dark:text-gray-300 hover:text-gray-900 hover:bg-gray-100 rounded transition duration-150 ease-in font-medium text-sm text-center w-full py-3">Email</a>
                                        </div>

                                        <div class="w-full dark:text-gray-300">
                                            <h3 class=" dark:text-gray-300 font-medium text-gray-900 text-left px-6">Your  Activites</h3>


                                            <div  class=" card pt-5 dark:bg-dark dark:text-light">

                                                <ul class="grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 w-90 dark:bg-dark dark:text-light" >
                                                    <% 
                                                        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM");
                                                        for (Map.Entry<String, Double> entry : monthlyTotalsMap.entrySet()) {
                                                            String monthString = entry.getKey();
                                                            double totalAmount = entry.getValue();
                                                            try {
                                                                // Parse the monthString to a Date object
                                                                java.util.Date date = dateFormat.parse(monthString);

                                                                // Format the Date object to a more readable format (e.g., "MMM yyyy")
                                                                String formattedMonth = new SimpleDateFormat("MMM yyyy").format(date);

                                                                // Use lorempixel for placeholder images
                                                                String iconPath = "https://source.unsplash.com/50x50/?business" + formattedMonth.substring(0, 3);

                                                    %>
                                                    <li class="bg-white rounded-md p-4 shadow-md flex items-center">
                                                        <img src="<%= iconPath %>" alt="<%= formattedMonth %> Icon" class="w-8 rounded-xl h-8 mr-2">
                                                        <div>
                                                            <p class="text-lg font-semibold text-gray-800"><%= formattedMonth %></p>
                                                            <p class="text-gray-600">Rs. <%= totalAmount %></p>
                                                        </div>
                                                    </li>
                                                    <% 
                                                            } catch (Exception e) {
